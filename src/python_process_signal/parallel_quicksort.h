...
int partition (int p, int r, long double *data)
{
	long double x = data[p];
	int k = p;
	int l = r + 1;
	long double t;
...
void seq_quick_sort (int p, int r, long double *data)
{
	if (p < r) {
...
void rec_quick_sort (int p, int r, long double *data, int low_limit)
{
	if (p < r) {
...
void quick_sort (long double *data, int n, int low_limit)
{
#pragma omp parallel
{
